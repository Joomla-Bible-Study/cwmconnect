...
namespace CWM\Component\Cwmconnect\Administrator\View\Kmls;

// phpcs:disable PSR1.Files.SideEffects
\defined('_JEXEC') or die;
// phpcs:enable PSR1.Files.SideEffects

use CWM\Component\Cwmconnect\Administrator\Model\KmlsModel;
...
/**
 * View class for a list of Kml records.
 *
 * @since  2.0.0
 */
class HtmlView extends BaseHtmlView
{
    ...
    protected function addToolbar(): void
    {
        ...
        ToolbarHelper::title(Text::_('COM_CWMCONNECT_MANAGER_KMLS'), 'cwmconnect');

        if ($canDo->get('core.create') || \count($user->getAuthorisedCategories('com_cwmconnect', 'core.create')) > 0) {
            $toolbar->addNew('kml.add');
        }

        if ($canDo->get('core.edit') || $canDo->get('core.edit.own')) {
            $toolbar->edit('kml.edit')->listCheck(true);
        }
        ...
        if ($canDo->get('core.edit.state')) {
            ...
            $childBar->publish('kmls.publish')->listCheck(true);
            $childBar->unpublish('kmls.unpublish')->listCheck(true);
            $childBar->archive('kmls.archive')->listCheck(true);
            $childBar->checkin('kmls.checkin')->listCheck(true);

            if ((int) $this->state->get('filter.published') !== -2) {
                $childBar->trash('kmls.trash')->listCheck(true);
            }
        }

        if ((int) $this->state->get('filter.published') === -2 && $canDo->get('core.delete')) {
            $toolbar->delete('kmls.delete', 'JTOOLBAR_EMPTY_TRASH')
