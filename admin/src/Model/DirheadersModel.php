...
/**
 * Methods supporting a list of Dirheader records.
 *
 * @since  2.0.0
 */
class DirheadersModel extends ListModel
{
    /**
     * Constructor.
     *
    ...
    protected function populateState($ordering = 'a.name', $direction = 'asc'): void
    {
        ...
        $this->setState(
            'filter.section',
            $this->getUserStateFromRequest($this->context . '.filter.section', 'filter_section', '', 'string')
        );

        $this->setState('params', ComponentHelper::getParams('com_cwmconnect'));
    ...
    protected function getListQuery(): QueryInterface
    {
        ...
        $query->from($db->quoteName('#__cwmconnect_dirheader', 'a'));
        ...
        if ($section !== '' && $section !== null) {
            $query->where($db->quoteName('a.section') . ' = ' . $db->quote($section));
        }
