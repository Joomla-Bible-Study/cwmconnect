...
class Com_ChurchdirectoryInstallerScript
{
	...
	private function setParams()
	{
		if (count($this->_param_array) > 0)
		{
			...
			$db    = JFactory::getDbo();
			$query = $db->getQuery(true);
			$query->select('params')->from('#__extensions')->where('name = ' . $db->quote($this->churchdirectory_extension));
			$db->setQuery($query);
			...
			$query        = $db->getQuery(true);
			$query->update('#__extensions')->set('params = ' . $db->quote($paramsString))->where('name = ' . $db->quote($this->churchdirectory_extension));
			$db->setQuery($query);
			$db->query();
		}
	}
