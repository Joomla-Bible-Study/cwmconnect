...
class ChurchDirectoryController extends JController
{
	...
	public function display($cachable = false, $urlparams = false)
	{
		...
		// Load the submenu.
		ChurchDirectoryHelper::addSubmenu(JRequest::getCmd('view', 'contacts'));
		...
		// Check for edit form.
		if ($view == 'contact' && $layout == 'edit' && !$this->checkEditId('com_churchdirectory.edit.contact', $id)) {

			// Somehow the person just went to the form - we don't allow that.
			$this->setError(JText::sprintf('JLIB_APPLICATION_ERROR_UNHELD_ID', $id));
			$this->setMessage($this->getError(), 'error');
			$this->setRedirect(JRoute::_('index.php?option=com_churchdirectory&view=contacts', false));
