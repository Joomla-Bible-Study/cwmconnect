 ...
 * @since		1.6
 */
class ChurchDirectoryModelContact extends JModelAdmin
{
	/**
	...
	protected function canDelete($record)
	{
		if (!empty($record->id)) {
			...
			return $user->authorise('core.delete', 'com_churchdirectory.category.'.(int) $record->catid);
		}
	}
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_churchdirectory.contact', 'contact', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_churchdirectory.edit.contact.data', array());
		...
		if (empty($data)) {
			...
			if ($this->getState('contact.id') == 0) {
				...
				$data->set('catid', JRequest::getInt('catid', $app->getUserState('com_churchdirectory.contacts.filter.category_id')));
			}
		}
