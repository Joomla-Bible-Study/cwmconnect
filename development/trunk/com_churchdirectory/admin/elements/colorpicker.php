...
class JElementColorpicker extends JElement {
    ...
    function fetchElement($name, $value, &$node, $control_name) {
        ...
        $scriptname = $node->attributes('scriptpath') ? $node->attributes('scriptpath') : $baseurl . 'media/com_churchdirectory/js/color-picker.js';
        ...
        $options = array();
        if ($node->attributes('cellwidth')) {
            $options[] = "cellWidth:" . (int) $node->attributes('cellwidth');
        }
        if ($node->attributes('cellheight')) {
            $options[] = "cellHeight:" . (int) $node->attributes('cellheight');
        }
        if ($node->attributes('top')) {
            $options[] = "top:" . (int) $node->attributes('top');
        }
        if ($node->attributes('left')) {
            $options[] = "left:" . (int) $node->attributes('left');
        }
