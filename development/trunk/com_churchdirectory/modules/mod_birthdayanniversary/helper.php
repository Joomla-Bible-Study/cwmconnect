...
class modBirthdayAnniversaryHelper {
    ...
    static function getBirthdays($params) {
        $db = JFactory::getDbo();
        $results = FALSE;
        $query = "SELECT * FROM " . $db->nameQuote('#__churchdirectory_details') . " WHERE published = 1 ORDER BY MONTH(birthdate) DESC";
    ...
    static function getAnniversary($params) {
        $db = JFactory::getDbo();
        $results = FALSE;
        $query = "SELECT * FROM " . $db->nameQuote('#__churchdirectory_details') . " WHERE published = 1 ORDER BY MONTH(anniversary) DESC";
        $records = modBirthdayAnniversaryHelper::performDB($query);
        foreach ($records as $record):
