...
class ChurchDirectoryViewPrintDirectory extends JView {
	function display($tpl = null)
	{
		...
		$pparams =& $mainframe->getParams('com_churchdirectory');

		$categoryId = JRequest::getVar('catid', 0, '', 'int');
		
		$limit = $mainframe->getUserStateFromRequest('com_churchdirectory.limit'.$categoryId, 'limit',$pparams->get('contacts_per_page', $mainframe->getCfg('list_limit')), '', 'int');
		$limitstart = JRequest::getVar('limitstart', 0, '', 'int');
		$limitstart = ($limit !=0 ? (floor($limitstart / $limit) * $limit): 0);
		$filter_order =$mainframe->getUserStateFromRequest('com_churchdirectory.filter_order'.$categoryId, 'filter_order', 'cd.'.$pparams->get('default_ordering','ordering'), '', 'cmd');
		$filter_order_Dir =$mainframe->getUserStateFromRequest('com_churchdirectory.filter_order_Dir'.$categoryId, 'filter_order_Dir', 'ASC', '', 'word');
		...
		if($pparams->get('show_col_image',1)) {
			$c = new stdClass;
			$c->field = 'image';
			$c->label ='Image';
		...
		if($pparams->get('show_col_lname',1)) {
			$c = new stdClass;
			$c->field = 'lname';
			$c->label ='Last Name';
			$c->sortable = $pparams->get('sort_col_lname',1);
			$c->width= $pparams->get('width_col_lname');;
			$columns[]=array('order'=>$pparams->get('ord_col_lname',0), 'column'=>$c);
		}
		...
		for($i = 0; $i < count($contacts); $i++)	{
			...
			$contact->link = JRoute::_('index.php?option=com_churchdirectory&view=contact&id='.$contact->slug.'&catid='.$contact->catslug);
		...
		if(!isset($category)){
			...
			$category->title = 'Church Directory';
			$category->image = '';
