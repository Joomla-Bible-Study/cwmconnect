...
class ChurchDirectoryViewChurchDirectory extends JView
{
	...
	function display($tpl = null)
	{
		...
		if ($item) {
			...
			$categoryModel = JModel::getInstance('Category', 'ChurchDirectoryModel', array('ignore_request' => true));
		...
		if ((!$active) || ((strpos($active->link, 'view=churchdirectory') === false) || (strpos($active->link, '&id=' . (string) $this->item->id) === false))) {
	...
	protected function _prepareDocument()
	{
		...
		if ($menu && ($menu->query['option'] != 'com_churchdirectory' || $menu->query['view'] != 'churchdirectory' || $id != $this->item->id))
		{
			...
			while ($category && ($menu->query['option'] != 'com_churchdirectory' || $menu->query['view'] == 'churchdirectory' || $id != $category->id) && $category->id > 1)
			{
