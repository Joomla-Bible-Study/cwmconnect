...
class ChurchDirectoryViewDirectory extends JView {
	function display()
	{
		...
		$document->link = JRoute::_('index.php?option=com_churchdirectory&view=directory'.JRequest::getVar('lname',null, '', 'int'));

		$limit 		= JRequest::getVar('limit', $mainframe->getCfg('feed_limit'), '', 'int');
		$limitstart = JRequest::getVar('limitstart', 0, '', 'int');
		$lname  	= JRequest::getVar('lname', 0, '', 'int');
		$where		= ' WHERE a.published = 1';

		if ( $catid ) {
			$where .= ' AND a.lname = '. (int) $lname;
		}
		...
		foreach ( $rows as $row )
		{
			...
			// url link to article
			$link = JRoute::_('index.php?option=com_churchdirectory&view=contact&id='. $row->slug .'&catid='.$row->catslug );
