<?php
/**
 * @version		$Id: churchdirectory.php 21991 2011-08-18 15:43:40Z infograf768 $
 * @package		Joomla.Site
	...
	public function submit()
	{
		...
		if($params->get('validate_session', 0)) {
			if(JFactory::getSession()->getState() != 'active'){
				JError::raiseWarning(403, JText::_('COM_CHURCHDIRECTORY_SESSION_INVALID'));

				// Save the data in the session.
				$app->setUserState('com_churchdirectory.churchdirectory.data', $data);

				// Redirect back to the churchdirectory form.
				$this->setRedirect(JRoute::_('index.php?option=com_churchdirectory&view=churchdirectory&id='.$stub, false));
				return false;
			}
		}
		...
		$app->setUserState('com_churchdirectory.churchdirectory.data', null);

		// Redirect if it is set in the parameters, otherwise redirect back to where we came from
		if ($churchdirectory->params->get('redirect')) {
			$this->setRedirect($churchdirectory->params->get('redirect'), $msg);
		} else {
			$this->setRedirect(JRoute::_('index.php?option=com_churchdirectory&view=churchdirectory&id='.$stub, false), $msg);
		}
