...
class ChurchDirectoryModelPrintDirectory extends JModel {
	
	function _getDirectoriesQuery( &$options ) {
		$db	=& JFactory::getDBO();
		$aid = @$options['aid'];

		$wheres[] = 'a.published = 1';
		$wheres[] = 'cc.section = ' . $db->Quote( 'com_churchdirectory_details' );
		...
		$query = 'SELECT cc.*, COUNT( a.id ) AS numlinks, a.id as cid'.
				...
				' LEFT JOIN #__churchdirectory_details AS a ON a.catid = cc.id'.
	...
	function _getDirectoryQuery(&$options){
		$aid = @$options['aid'];
		$catid = @$options['category_id'];
		$wheres[] = 'published = 1';
		$wheres[] = "section = 'com_churchdirectory_details'";
		$wheres[] = "id = " . (int) $catid;
		if ($aid !== null) {
	...
	function _getContactsQuery( &$options )	{
		...
		$from	= '#__churchdirectory_details AS cd';
	...
	function getDirectory($options=array()){
		$query	= $this->_getDirectoryQuery( $options );
		$result = $this->_getList( $query );
		return @$result[0];
	}
	function getDirectories( $options=array() ) {
		$query	= $this->_getDirectoriesQuery( $options );
		return $this->_getList( $query, @$options['limitstart'], @$options['limit'] );
	}

	function getDirectoryCount( $options=array() )	{
		$query	= $this->_getDirectoriesQuery( $options );
